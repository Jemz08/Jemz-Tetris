...
function drawCell(ctx, x, y, size, color, flash = 0) {
  ...
  ctx.fillRect(x0, y0, s, Math.max(1, s * 0.16));
  ctx.fillRect(x0, y0, Math.max(1, s * 0.16), s);
  // bottom shadow
  ctx.fillStyle = 'rgba(0,0,0,0.35)';
  ctx.fillRect(x0, y0 + s - Math.max(1, s * 0.14), s, Math.max(1, s * 0.14));
  ctx.fillRect(x0 + s - Math.max(1, s * 0.14), y0, Math.max(1, s * 0.14), s);
}
...
export function drawMiniPiece(canvas, type, dim = true) {
  ...
  for (const [r, c] of cells[type]) {
    ctx.fillStyle = dim ? 'rgba(255,255,255,0.35)' : COLORS[type];
    ctx.fillRect(ox + c * cell, oy + r * cell, cell - 1, cell - 1);
    if (!dim) {
      ctx.fillStyle = 'rgba(255,255,255,0.35)';
      ctx.fillRect(ox + c * cell, oy + r * cell, cell - 1, Math.max(1, (cell - 1) * 0.2));
    }
  }
}
...
export function updateArenaHud(controller) {
  controller.players.forEach((p, i) => {
    ...
    if (i === 0) {
      ...
      if (next) drawMiniPiece(next, p.queue.length ? p.queue[0].type : null);
    }
  });
}
